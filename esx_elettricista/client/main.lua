...
function StopNPCJob(cancel)
    ...
    if cancel then
	  ESX.ShowNotification(_U('cancel_mission'))
		...
		TriggerServerEvent('esx_elettricista:GiveItem')
		StartNPCJob()
		Done = true
	end
end
            ...
            local playerPed = GetPlayerPed(-1)
           	...
           	if GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < 3 then
                ...
                if IsControlJustReleased(1, Keys["E"]) and PlayerData.job ~= nil then
					...
					exports['progressBars']:startUI(10000, "Ispezionando la rete")
					Wait(10000)
					TaskStartScenarioInPlace(playerPed, "CODE_HUMAN_POLICE_INVESTIGATE", 0, true)
					Wait(12000)
					TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_WELDING", 0, true)
					exports['progressBars']:startUI(17000, "Aggiustando la rete")
					Wait(17000)
                    StopNPCJob()
                    Wait(3000)
...
AddEventHandler('esx_elettricista:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_elettricista:hasExitedMarker', function(zone)

	if zone == 'Vente' then
		TriggerServerEvent('esx_elettricista:stopVente')
	end
    CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_elettricista:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_elettricista:hasExitedMarker', LastZone)
			end
		end
	end
end)
            ...
            if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
				...
				if PlayerData.job.name == Config.nameJob then
					...
					if CurrentAction == 'vente' then
						TriggerServerEvent('esx_elettricista:startVente')
	...
	while true do
		...
		if IsControlJustReleased(1, Keys["F10"]) and PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then
			...
			if Onjob then
				...
				if IsPedInAnyVehicle(playerPed,  false) and IsVehicleModel(GetVehiclePedIsIn(playerPed,  false), GetHashKey("speedo")) then
					StartNPCJob()
					Onjob = true
				else
